...
export default function ProvidersPage() {
  ...
  const allProviders = channels.flatMap((c) =>
    c.providers.map((p) => ({ ...p, channel_id: c.channel_id, channel_name: c.channel_name }))
  );
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <Input
                ...
                className="h-9 w-64 pl-8 text-sm bg-white"
              />
            </div>
            <div className="relative">
              <select
                ...
                className="h-9 appearance-none rounded-md border bg-white px-3 pr-8 text-sm"
              >
      ...
      <div className="space-y-8">
        {channels
          .filter((channel) => !hasChannelFilter || channel.channel_id === selectedChannel)
          .map((channel) => {
          const providers = channel.providers.filter((p) => {
            if (hasSearch && !p.name.toLowerCase().includes(searchLower)) return false;
            return true;
          });
          ...
          return (
            <div key={channel.channel_id} className="space-y-3">
              <div className="flex items-center gap-2">
                <span className="text-lg">{CHANNEL_ICONS[channel.channel_id] ?? "📦"}</span>
                  ...
                  >
                    <CardHeader className="pb-2">
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <div className={`flex size-8 items-center justify-center rounded-lg bg-muted text-xs font-bold ${provider.icon_url ? "hidden" : ""}`}>
                    ...
                    <CardContent className="space-y-2 flex-1 flex flex-col">
                      ...
                      <Button
                        ...
                        onClick={() => setConnectDialog({ channel: channel.channel_id, provider: provider.provider_id })}
                      >
        ...
        {hasSearch && channels.every((c) =>
          c.providers.every((p) => !p.name.toLowerCase().includes(searchLower))
        ) && (
      ...
      >
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            {connectDialog && (() => {
              const channel = channels.find((c) => c.channel_id === connectDialog.channel);
              const provider = channel?.providers.find((p) => p.provider_id === connectDialog.provider);
