...
```json
{ "email": "string", "password": "string", "rememberMe": "boolean", "totpCode?": "string" }
```
...
OR `200 { "requiresTotp": true }` (no session, no cookie) when the account has 2FA enabled and no `totpCode` was sent — the page switches to its OTP step, which completes via `totpCode` here or `POST /totp/challenge` (no password resend).
...
### 1b. TOTP challenge — `POST /app/auth/totp/challenge`

Request: `{ "email": "string", "code": "string" }`

Completes login for a TOTP-enabled account without a password (used after the OAuth detour, which can't carry one). Issues a 1-day session + cookie. Errors: `401` unknown account / bad code, `403` suspended, `400` when 2FA isn't enabled. The OAuth callback redirects TOTP accounts to `/auth/login?totp=1` (popup mode posts `{type:"oauth:totp_required"}`); the OTP step asks for email + code.

### 1c. TOTP setup / verify / disable

- `POST /app/auth/totp/setup` (auth) → `200 { "totpSecret", "totpUri" }`. Get-or-create: reuses the pending secret until verified. `400` while already enabled (disable first).
- `POST /app/auth/totp/verify` (auth) `{ "code" }` → `200 {status:"ok"}`, enables 2FA. `401` on wrong code.
- `POST /app/auth/totp/disable` (auth) `{ "password?": "string", "code": "string" }` → `200 {status:"ok"}`. Password required when the account has one (OAuth-only accounts use code alone). Wired to the profile page Security section.
...
#### `PATCH /app/projects/:id/require-2fa` — 2FA gate for new members
...
```ts
User    = { id, name, email, avatar: string | null, emailVerified, totpEnabled, createdAt, lastLoginAt: string | null }
