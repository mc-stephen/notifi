...
async fn create_and_list_and_get_and_delete_roundtrip() {
    let (app, _store) = app_with_recipients();
    let (token, _uid, pid) =
        onboard_project_on(app.clone(), &_store, "rcp1@example.com", "My App").await;
    ...
    .await;
    assert_eq!(
        body_json(res).await["recipients"].as_array().unwrap().len(),
        0
    );
}
...
async fn duplicate_user_id_within_a_project_conflicts() {
    let (app, _store) = app_with_recipients();
    let (token, _uid, pid) =
        onboard_project_on(app.clone(), &_store, "rcp2@example.com", "My App").await;
...
async fn recipients_are_isolated_per_project() {
    ...
    .await;
    assert_eq!(res.status(), StatusCode::CREATED);
    let rid = body_json(res).await["recipient"]["id"]
        .as_str()
        .unwrap()
        .to_string();
    ...
    let res = get_with_cookie(
        app.clone(),
        &format!("/app/projects/{pid_b}/recipients"),
        &token_b,
    )
    .await;
    assert_eq!(res.status(), StatusCode::OK);
    ...
    let res = get_with_cookie(
        app.clone(),
        &format!("/app/projects/{pid_b}/recipients/{rid}"),
        &token_b,
    )
    .await;
...
async fn create_requires_user_id_and_name() {
    let (app, store) = app_with_recipients();
    let (token, _uid, pid) =
        onboard_project_on(app.clone(), &store, "rcp4@example.com", "My App").await;
    ...
    let res = post_with_cookie(
        app.clone(),
        &uri,
        json!({"userId": "u_x", "name": "Bad contacts", "contacts": [1, 2]}),
        &token,
    )
    .await;
...
async fn update_changes_name_and_replaces_contacts() {
    let (app, store) = app_with_recipients();
    let (token, _uid, pid) =
        onboard_project_on(app.clone(), &store, "rcp5@example.com", "My App").await;
    ...
    assert!(
        updated["contacts"]["phone"].is_null(),
        "phone should be replaced away"
    );
...
async fn update_name_only_keeps_contacts() {
    let (app, store) = app_with_recipients();
    let (token, _uid, pid) =
        onboard_project_on(app.clone(), &store, "rcp6@example.com", "My App").await;
...
async fn update_unknown_recipient_404_and_bad_contacts_400() {
    let (app, store) = app_with_recipients();
    let (token, _uid, pid) =
        onboard_project_on(app.clone(), &store, "rcp7@example.com", "My App").await;
