...
| Migrations | sqlx (single `migrations/` set) |
...
└── crates/
    ...
    ├── infra/
    │   ├── postgres/             # notifi-infra-postgres: pool, runner, health
    │   ├── redis/                # notifi-infra-redis: rate limit, cache, pub/sub
    │   ├── queue/                # notifi-infra-queue: pgmq producer/consumer, DLQ
    │   ├── telemetry/            # notifi-infra-telemetry: tracing, OTel, metrics
    │   └── config/               # notifi-infra-config: layered loader
    ├── domains/
    │   ├── organizations/        # orgs, projects, environments
    │   ├── auth/                 # api keys, sessions, members
    │   ├── notifications/        # notification aggregate + state machine
    │   ├── delivery/             # dispatch, provider registry, retries, DLQ
    │   ├── recipients/           # recipients, devices, channel preferences
    │   ├── providers/            # provider accounts + channel configs
    │   ├── templates/            # template render + publish
    │   ├── webhooks/             # outgoing webhook delivery
    │   ├── schedules/            # scheduled/cron sends
    │   ├── analytics/            # event read models
    │   └── billing/              # usage metering, plans
    ├── api/                      # notifi-api (binary)
    ├── workers/                  # notifi-workers (binary)
    └── adapters/
...
- Single `migrations/` set, numbered `0001_xxx.sql`, run at startup and via
  CLI (`sqlx migrate`).
