...
use providers::{TwilioProvider, LocalNigeriaProvider, SmsSender};
...
impl SmsProvider {
    pub fn new(config: SmsConfig) -> Self {
        let sender: Box<dyn SmsSender> = match config {
            SmsConfig::Twilio { account_sid, auth_token, from_number } => {
                Box::new(TwilioProvider::new(account_sid, auth_token, from_number))
            }
            SmsConfig::LocalNigeria { api_key, base_url, sender_id } => {
